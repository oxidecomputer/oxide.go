...
// Variant represents a single variant in a tagged union.
type Variant struct {
	...
	TypeName string
	// Format is set for format-based discrimination on string types (e.g., "ipv4", "ipv6").
	Format string
	// Pattern is set for pattern-based discrimination on string types (regex).
	Pattern string
	// FormatFields holds fields with format constraints for struct validation.
	FormatFields []FormatField
	// PatternFields holds fields with pattern constraints for struct validation.
...
// PatternField describes a field with a pattern constraint for validation.
...
func createOneOf(s *openapi3.Schema, name, typeName string) ([]TypeTemplate, []EnumTemplate) {
	...
	if analysis := checkUntaggedOneOf(s); analysis != nil {
		return createUntaggedOneOf(s, name, typeName, analysis)
	}
...
// checkUntaggedOneOf checks if a oneOf schema represents an untagged union
// that can be discriminated by format or pattern constraints.
//
...
//
// Returns nil if the schema is not a discriminable untagged union.
