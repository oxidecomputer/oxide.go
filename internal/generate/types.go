...
type TypeTemplate struct {
	...
	OneOfMarker string
	// OneOfMarkerType is the interface type name for variant structs (e.g., "addressSelectorVariant")
	...
	// OneOfVariants holds discriminator value -> variant type name mapping for JSON marshal/unmarshal
...
func createOneOf(s *openapi3.Schema, name, typeName string) ([]TypeTemplate, []EnumTemplate) {
	// First pass: identify discriminator key and find properties with multiple types across variants.
	...
	if len(discriminatorKeys) == 1 && len(multiTypeProps) == 1 {
		return createInterfaceOneOf(s, name, typeName, discriminatorKeys, multiTypeProps)
	}

	// Otherwise, use flat struct pattern (original behavior).
	return createFlatOneOf(s, name, typeName, discriminatorKeys, multiTypeProps)
}
...
func createInterfaceOneOf(s *openapi3.Schema, name, typeName string, discriminatorKeys, multiTypeProps map[string]struct{}) ([]TypeTemplate, []EnumTemplate) {
	enumTpls := make([]EnumTemplate, 0)
	typeTpls := make([]TypeTemplate, 0)

	// Get the discriminator key name
	...
	// Get the multi-type property name
	...
	for _, variantRef := range s.OneOf {
		...
		enums, tt, et := createStringEnum(discRef.Value, collectEnumStringTypes, discriminatorType, discriminatorType)
		collectEnumStringTypes = enums
		...
		if hasValue {
			...
			if valueRef.Ref == "" && valueRef.Value != nil && valueRef.Value.Type != nil && valueRef.Value.Type.Is("object") {
...
func createFlatOneOf(s *openapi3.Schema, name, typeName string, discriminatorKeys, multiTypeProps map[string]struct{}) ([]TypeTemplate, []EnumTemplate) {
	enumTpls := make([]EnumTemplate, 0)
	typeTpls := make([]TypeTemplate, 0)
